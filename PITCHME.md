...
fun sayHi(greeting: String = "Howdy",
          name: String = "Honored Guest"): Unit {
...
Note:
"Unit" here is the same as "void" in java
...
Note:
Explain property access, generated getters & setter
...
Note:
Yeah, your IDE can generate all this stuff. 
But it still adds an additional maintenance burden, plus differentiating vanilla boilerplate and special cases is hard.
  
+++

## java interop for classes
```java
public static void main(String[] args) {
    Name name = new Name("my", "name", "middle");
    System.out.println(name.getFirst());
}
```

Note: 
kotlin has properties, but the generated bytecode has java-friendly getters & setters
...
Note:
instanceof + cast is unnecessary - the compiler can do this work for you.
...
sealed class Failure : ServiceResult<Nothing>() {
    ...
    data class Retriable(val message: String, 
                         val retriesRemaining: Int): Failure()
...
class ExDelegate {
    operator fun  getValue(ref: Example, 
                           property: KProperty<*>): Any {
        return "$ref delegated '${property.name}' to me"
    }

    operator fun setValue(ref: Example, 
                          property: KProperty<*>, value: Any) {
        println("$value assigned to '${property.name} in $ref.'")
    }
}
```
...
```kotlin
// `build` is a locally extension function 
// on a HashMap
